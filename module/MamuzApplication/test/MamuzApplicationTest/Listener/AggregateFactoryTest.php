...
use MamuzApplication\Listener\AggregateFactory;
...
class AggregateFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $cache = \Mockery::mock('MamuzApplication\Service\Feature\OutputCacheInterface');
        $sm = \Mockery::mock('Zend\ServiceManager\ServiceLocatorInterface');
        $sm->shouldReceive('get')
            ->with('MamuzApplication\PluginManager')
            ->andReturn($sm);
        $sm->shouldReceive('get')->with('MamuzApplication\Service\OutputCache')->andReturn($cache);
